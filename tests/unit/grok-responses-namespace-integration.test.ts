...
for (const provider of ["grok-cli", "xai"]) {
  for (const stream of [false, true]) {
    for (const custom of [false, true]) {
      test(`${provider} namespace tools round-trip through native Responses (stream=${stream}, custom=${custom})`, async () => {
        const tools = [
          {
            type: "namespace",
            name: "files",
          ...
          input: [
            {
              ...
              name: "read",
              call_id: "old_call",
              ...(custom ? { input: "previous text" } : { arguments: "{}" }),
            },
            {
        ...
        globalThis.fetch = async (_url, init) => {
          ...
          if (sentTools.some((t) => t.type === "namespace" || t.type === "custom")) {
            return new Response(
              JSON.stringify({ error: { message: "unknown variant namespace" } }),
              {
                status: 422,
              }
            );
          ...
          const call = {
            ...
            call_id: "call_new",
            name: "files__read",
            arguments: custom ? JSON.stringify({ input: "hello\nworld" }) : "{}",
            status: "completed",
          };
          const response = {
            id: "resp_test",
          ...
          const frames = [
            ...
            },
            {
              ...
              item_id: "fc_new",
              output_index: 0,
              delta: call.arguments.slice(0, 5),
            },
            {
              type: "response.function_call_arguments.delta",
              item_id: "fc_new",
              output_index: 0,
              delta: call.arguments.slice(5),
            },
            {
        ...
        try {
          ...
          for (const output of outputs) {
            ...
            assert.equal(output.type, custom ? "custom_tool_call" : "function_call");
            if (custom && output.status === "completed") assert.equal(output.input, "hello\nworld");
          }
          if (custom && stream) {
            assert.ok(raw.includes("response.custom_tool_call_input.delta"));
            assert.ok(raw.includes("response.custom_tool_call_input.done"));
            assert.ok(!raw.includes("response.function_call_arguments.delta"));
          }
